...
public class ResenjeService implements AbstractXmlService<Resenje> {
    ...
    public Resenje create(String xmlEntity) {
        ...
        String email = "";
        if (resenje.getTipZalbe().equals("zalba_na_odluku")) {
            email = this.zalbaNaOdlukuService.findEmailGradjaninaByIdZalbe(resenje.getIdZalbe());
        } else {
            email = this.zalbaNaCutanjeService.findEmailGradjaninaByIdZalbe(resenje.getIdZalbe());
        }
        this.emailService.sendResenje(
                resenje,
                email,
                this.generatePdf(resenje.getId()),
                this.generateHtml(resenje.getId())
        );

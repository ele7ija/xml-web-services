<template>
  <div class="container">
    <div class="row">
      <div class="col-12">
        <h3 class="text-center mt-5 mb-4">Osnov za zalbu na odluku</h3>
    ...
    <div class="row">
      <div class="col-12">
        <h3 class="text-center mt-5 mb-4">Podnete zalbe na odluku</h3>
        <table class="table table-sm table-bordered">
          <thead>
...
import { constructKolekcijaObavestenja, constructKolekcijaZalbi } from '../../../util';
...
export default {
  ...
  async mounted() {
    if (sessionStorage.getItem('access_token')) {
      ...
      this.obavestenja = constructKolekcijaObavestenja((await obavestenjeApi.getOdbijena()).data);
      this.zalbe = constructKolekcijaZalbi((await zalbaNaOdlukuApi.getAllUlogovanogKorisnika()).data);
      this.obavestenjaLoading = false;
    }
  },
  methods: {
    navigate (obavestenje) {
      this.$router.push({path: `zalba-na-odluku-create/${obavestenje.id}`});
    },

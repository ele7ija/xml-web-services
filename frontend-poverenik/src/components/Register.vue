      ...
      <div class="row">
        <div class="col-12">
          ...
          <ul class="nav nav-pills nav-fill my-5">
            ...
            <li class="nav-item">
              <span :class="'nav-link '+(registerGradjanin ? '' : 'active')" @click="switchToSluzbenikView" :style="{cursor: 'pointer'}">Poverenik</span>
            </li>
          </ul>
...
export default {
  ...
  methods: {
    ...
    switchToSluzbenikView () {
      ...
      const xmlString = 
        `<?xml version="1.0" encoding="UTF-8"?>
          <pv:Poverenik xmlns:pv="http://ftn.uns.ac.rs/tim5/model/poverenik"
              xmlns:util="http://ftn.uns.ac.rs/tim5/model/util"
              xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
              <pv:korisnicko_ime></pv:korisnicko_ime>
              <pv:lozinka></pv:lozinka>
              <pv:podaci>
                  <util:Adresa>
                      <util:Mesto></util:Mesto>
                      <util:Ulica></util:Ulica>
    ...
    async register() {
      ...
      if (this.registerGradjanin) {
        ...
        await loginApi.createPoverenik(Xonomy.harvest());
      }
